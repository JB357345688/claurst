...
| Field | Value |
|-------|-------|
| **RFC ID** | RFC-0001-v3.1 |
| **Status** | Draft |
| **Author** | Claurst Core Team |
| **Date** | 2026-04-08 |
| **Supersedes** | RFC-0001-v3, RFC-0001-v2, RFC-0001-v1 |
| **Codebase Version** | `acae926` (2026-04-07) |
| **Affects** | `claurst-query`, `claurst-api`, `claurst-tools`, `claurst-core` |
| **Review History** | v1 reviewed by Codex/GPT-5.4; v2 reviewed by Codex/GPT-5.4; v3 incorporates all v2 review blockers; v3.1 tightening pass — uniform explicit-provider semantics, D2 trust-domain scope narrowing |
...
**Deliverable 1 (D1 — Minimal Fix, ~900 LoC):** Extract the existing provider-resolution logic from `run_query_loop()` into a two-phase resolution pipeline: `resolve_provider_identity()` (pure logic: inputs → provider ID + model ID) and `materialize_provider()` (side-effectful: auth-store refresh, api_base overrides → `Arc<dyn LlmProvider>`). Propagate `ProviderRegistry` and `ModelRegistry` into child `QueryConfig` for both `AgentTool` and `TeamCreateTool`. Includes a normative precedence matrix for all provider/model combinations, uniform explicit-provider semantics (all provider IDs, including `"anthropic"`, are true pins), and strict error semantics (no silent Anthropic fallback when a registry is present).

**Deliverable 2 (D2 — Expansion, ~1,400 LoC):** Add soft-budget enforcement via `SessionBudget`, spawn-time health checks via `HealthCache`, spawn-time fallback within trust domains (scoped to built-in provider IDs), capability matching with explicit unknown-data policy, and cancellation propagation with defined token ownership.

v2 of this RFC was reviewed and produced 7 blocking findings. v3 addresses all 7: normative precedence matrix (B1), strict fallback semantics (B2), two-phase resolution contract (B3), explicit `allow_cross_domain_fallback` placement (B4), deferred custom trust-domain support (B5), capability unknown-data policy (B6), and cancellation propagation specification (B7). v3.1 further tightens B1 with uniform explicit-provider semantics (removing the `config_provider = "anthropic"` special case from the precedence matrix) and narrows D2 trust-domain scope language to be explicit about the limits of built-in-only classification.
...
**Note:** Step 1 currently applies a `filter(|p| *p != "anthropic")` guard at `lib.rs:863`, causing `config.provider = "anthropic"` to be skipped and treated as `None`. D1 removes this exception — see Section 7.1.1 for the normative resolution semantics where all explicit provider values are true pins.
...
| NG9 | **Generalized self-hosted OpenAI-compatible provider support in trust domains.** D2 trust-domain classification and fallback are defined for built-in provider IDs only. Self-hosted endpoints using OpenAI-compatible APIs are not first-class participants in D2 trust-domain logic until the deferred `ProviderConfig` extension is implemented. | Scope / v2 blocker #5 |
...
| **Trust domain** | A classification of providers by data-handling boundary: `Local` (Ollama, LM Studio, llama.cpp — data never leaves the machine), `Cloud` (all others). Fallback never crosses trust-domain boundaries unless explicitly opted in. D2 trust-domain classification is defined for built-in provider IDs only (see Section 7.4). |
| **Pinned provider** | An explicit `provider` in the worker request. The system must use this provider or fail — no fallback. Default behavior. All provider IDs are treated uniformly — there is no special case for `"anthropic"` or any other ID. |
...
5. **Pinned by default.** An explicit `provider` means "use this provider or fail." This applies uniformly to all provider IDs, including `"anthropic"`. Fallback is opt-in via `allow_fallback: true`.
...
**Rows P1-P6 apply to any explicit `config_provider`, including `"anthropic"`.** There is no special case for any provider ID.
...
| P5 | `Some("anthropic")` | `"openai/gpt-4.1"` (conflicting known prefix) | **Error: `ProviderModelConflict`** | — |
| P6 | `Some("anthropic")` | `"gemini-3-flash-preview"` (bare, in registry as google) | `(anthropic, "gemini-3-flash-preview")` | `ExplicitProvider` |
| P7 | `None` | `"openai/gpt-4.1"` (known prefix) | `(openai, "gpt-4.1")` | `ModelStringPrefix` |
| P8 | `None` | `"meta-llama/Llama-3.3"` (unknown prefix) | Fall through to ModelRegistry; if no hit, `(anthropic, "meta-llama/Llama-3.3")` | `ModelRegistry` or `Default` |
| P9 | `None` | `"gemini-3-flash-preview"` (bare, in registry) | `(google, "gemini-3-flash-preview")` | `ModelRegistry` |
| P10 | `None` | `"claude-opus-4-6"` (bare, in registry as anthropic) | `(anthropic, "claude-opus-4-6")` | `ModelRegistry` or `Default` |
| P11 | `None` | `"some-unknown-model"` (bare, not in registry) | `(anthropic, "some-unknown-model")` | `Default` |
| P12 | `None` | `""` (empty) | **Error: `NoProviderForModel`** | — |

**Conflict detection rule (P3, P5):** When `config_provider` is set to any value and the model string has a known provider prefix that differs from `config_provider`, this is an unambiguous conflict. The function returns `ProviderModelConflict`. This applies uniformly to all provider IDs — `provider: "anthropic"` with `model: "openai/gpt-4.1"` is the same class of conflict as `provider: "openai"` with `model: "anthropic/claude-opus-4-6"`. This is a **new behavior** not present in the current inline code, which would silently use `config_provider` and strip a mismatched prefix (or, for `"anthropic"`, skip the explicit-provider branch entirely).

**Uniform explicit-provider semantics (v3.1 change):** All explicit `config_provider` values are true pins. There is no special case for `"anthropic"` or any other provider ID. When `config_provider` is `Some(P)`, the resolution is always `(P, model)` for bare models or matching prefixes, and `ProviderModelConflict` for conflicting known prefixes.

**Behavioral change from current code:** The current inline code at `lib.rs:863` applies a `filter(|p| *p != "anthropic")` guard that causes `config_provider = "anthropic"` to be treated as `None`, falling through to the model-string-prefix and ModelRegistry branches. D1 removes this exception. The practical impact is narrow: it only affects the case where `--provider anthropic` is set *and* the model string has a non-Anthropic known prefix (e.g., `openai/gpt-4.1`). In current code, the prefix would win silently; under this RFC, this is a `ProviderModelConflict` error (P5). For bare model names, the outcome is unchanged in practice because `ExplicitProvider` resolution yields the same `(anthropic, model)` pair that the `Default` path would. **P6 is the key illustration:** `provider: "anthropic"` with `model: "gemini-3-flash-preview"` now resolves to `(anthropic, "gemini-3-flash-preview")` via `ExplicitProvider`, not to `(google, ...)` via `ModelRegistry`. The explicit pin is honored.
...
**Consequence for fallback candidacy:** Because custom providers with unrecognized IDs default to `TrustDomain::Cloud`, they will appear as Cloud-domain fallback candidates if they are registered in the `ProviderRegistry` and pass health/capability checks. This may be surprising for self-hosted OpenAI-compatible endpoints that are actually local. Until the deferred `trust_domain` config extension is implemented, users deploying self-hosted providers should explicitly pin workers to those providers rather than relying on fallback to discover them. See also Section 7.5, "Fallback candidate scope."
...
**Fallback candidate scope:** Fallback candidates are drawn from all providers registered in the `ProviderRegistry` that share the primary target's trust domain (per `TrustDomain::for_provider()`) and pass capability/health checks. This includes custom providers whose IDs are not in the built-in local list, because such providers default to `TrustDomain::Cloud` (Section 7.4). D2 does not implement a mechanism to exclude specific providers from fallback candidacy or to override their trust-domain classification. If a registered custom provider should not be a fallback target, workers should be explicitly pinned (`allow_fallback: false`, the default) to their intended providers. A more granular fallback-exclusion or trust-domain-override mechanism is deferred along with custom trust-domain support (NG8, NG9).
...
| `provider/model` conflict now returns explicit error (P3/P5) | New error path | D1 |
| `provider: "anthropic"` is a true pin (no longer treated as `None`) | Behavioral change from current code (see Section 7.1.1) | D1 |
...
| F3 | Provider/model conflict (e.g., `provider=openai` + `model=anthropic/claude-opus-4-6`, or `provider=anthropic` + `model=openai/gpt-4.1`) | D1 | `ProviderResolutionError::ProviderModelConflict` with clear message |
...
**New error path (P3/P5):** A user who passes a conflicting provider and model prefix will now get an explicit `ProviderModelConflict` error. For example, `provider: "openai"` with `model: "anthropic/claude-opus-4-6"` (P3), or `provider: "anthropic"` with `model: "openai/gpt-4.1"` (P5). Previously, the inline code would silently use `config_provider` and strip a mismatched prefix. This is technically a behavior change, but the old behavior was almost certainly not what the user intended.

**Uniform pin semantics for `"anthropic"` (v3.1 change):** The current code at `lib.rs:863` treats `config_provider = "anthropic"` as `None` (via a `filter` guard). D1 removes this exception. The practical impact is narrow: it only matters when `--provider anthropic` is set *and* the model string has a non-Anthropic known prefix (e.g., `openai/gpt-4.1`). In current code, the prefix would win silently; under this RFC, this is a `ProviderModelConflict` error. For bare model names or Anthropic-prefixed models, the outcome is unchanged. This change is intentional: an explicit provider selection should mean what it says, regardless of which provider it names.
...
| 1.13 | Unit tests for `resolve_provider_identity()` — all precedence matrix rows (P1-P12) | `crates/query/src/provider_resolution.rs` (tests module) | ~180 LoC |
...
**Verification:** `cargo test --workspace` passes. Every row of the precedence matrix (P1-P12) has a corresponding unit test. Agent can be spawned with `provider: "openai"`. Agent without `provider` inherits parent's provider.
...
| # | Criterion | Type |
|---|----------|------|
| AC1 | `resolve_provider_identity()` matches precedence matrix rows P1-P12 exactly | Unit test per row |
...
| AC8 | Agent with conflicting provider and model prefix returns `ProviderModelConflict` error — both directions: `provider: "openai"` + `model: "anthropic/claude-opus-4-6"` (P3), and `provider: "anthropic"` + `model: "openai/gpt-4.1"` (P5) | Unit test |
...
|-----------|-----------|----------|
| `resolve_provider_identity()` | All 12 precedence matrix rows, empty model, model with multiple slashes | Mock `ProviderRegistry` with 3-4 providers |
...
| Provider/model conflict (reverse) | `provider: "anthropic"` + `model: "openai/gpt-4.1"` | `ProviderModelConflict` error |
...
**Note on fallback candidacy:** Because custom providers with unrecognized IDs default to `TrustDomain::Cloud`, they may appear as Cloud-domain fallback candidates in D2 if they are registered in the `ProviderRegistry`. This is a known limitation of the built-in-only trust-domain model, not an intended feature. See Section 7.4 and Section 7.5 ("Fallback candidate scope") for details.
...
D1 is a **bug fix**. Workers should inherit their parent's provider configuration. The fix is small (~900 LoC), low-risk, and immediately useful. The two-phase resolution preserves all existing root behavior (auth-store refresh, api_base overrides) while making the logic reusable. The normative precedence matrix eliminates ambiguity. Uniform explicit-provider semantics ensure that `provider: "anthropic"` means the same thing as `provider: "openai"` — a true pin.
...
| Self-hosted OpenAI-compatible provider trust-domain classification | Requires custom trust-domain override (see above); not generalizable with built-in-only classification |
...
| Backward-compat risk | **Low** — behavior change is from "fail" to "succeed"; new P3/P5 conflict error and uniform anthropic pin semantics are both improvements over silently wrong behavior | **Minimal** — new fields are optional |
| Scope risk | **Minimal** — ~900 LoC, well-understood code; slightly higher than v2 estimate due to two-phase split and conflict detection | **Low-Medium** — ~1,400 LoC; cancellation propagation adds complexity but is well-defined. Custom providers may appear as unintended Cloud fallback candidates (see Section 7.4); mitigated by default `allow_fallback: false` and explicit pinning. |
...
| # | v2 Blocker | Severity | Disposition in v3/v3.1 |
|---|-----------|----------|-------------------|
| B1 | Provider/model precedence is contradictory | Blocking | **Resolved.** Added normative precedence matrix (Section 7.1.1) with 12 explicit rows covering all input combinations. Added `ProviderModelConflict` error variant. The matrix is declared authoritative over any other section. v3.1 further tightened this by removing the `config_provider = "anthropic"` special case: all explicit provider values are now true pins with uniform semantics (P5, P6). |
...
| B5 | Custom local-provider trust-domain support is hand-wavy | Blocking | **Resolved.** Explicitly deferred. D2 only supports built-in provider IDs. The fake config example from v2 OQ6 is removed. A follow-on RFC will extend `ProviderConfig` with `trust_domain` and define the custom provider contract. v3.1 additionally documents the fallback-candidacy consequence of the Cloud default for custom providers (Section 7.4, Section 7.5, OQ6). (Section 5.3, Section 7.4, NG8, NG9) |
...
*End of RFC v3.1. This document supersedes RFC-0001-v3, RFC-0001-v2, and RFC-0001-v1. It incorporates all 7 blocking findings from the v2 adversarial review (2026-04-08), all non-blocking issues, and the v3.1 tightening pass (uniform explicit-provider semantics, D2 trust-domain scope narrowing).*
