...
> Status: historical planning context only / non-controlling.
>
> This planning document is retained for historical context and is not the active authority for post-M11 work.
> Active current authority now lives in `docs/Current/MPWO_WORK_ORDER_PACK.md`.
> Use this file only when a prompt explicitly asks for historical planning traceability.
>
> Traceability:
