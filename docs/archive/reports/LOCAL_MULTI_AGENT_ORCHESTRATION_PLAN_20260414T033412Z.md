# Local Multi-Agent Orchestration Plan
...
- Relevant CLI surfaces are usable locally: `codex exec` supports stdin prompts, `--json`, `--output-schema`, `-o`, `--sandbox`, `--full-auto`, `--ephemeral`; `claude` supports `-p`, `--output-format json`, `--permission-mode`, `--effort`, `--bare`, `--no-session-persistence`; `gemini` supports `--prompt`, `--output-format json`, `--approval-mode`, `--sandbox`, `--model`.
...
- `.gitignore` is minimal and does not yet ignore an orchestration runtime directory; that must be added in the implementation run.
...
- Default tool choice on this machine should be `codex` for both roles. Reviewer default: `codex` with `gpt-5.4`, read-only sandbox, high-reasoning profile/override. Drafter default: `codex` with `gpt-5.4-mini`, workspace-write sandbox, fast/medium profile/override.
- Fallback order should be `claude` first, then `gemini`. `claude` is acceptable because it has `--bare` and `--no-session-persistence`; `gemini` stays secondary because its hidden-state controls are weaker and need stricter adapter isolation.
...
- Every rendered prompt starts with a deterministic header containing `run_id`, `cycle`, `stage`, `workspace`, `git_head`, `provider`, `model`, `sandbox`, `approval`, `reasoning`, referenced input files, and SHA256 digests of those inputs.
...
5. `resolve-config`: choose reviewer/drafter adapters from installed tools; write `resolved_config.json`.
6. `render-blueprint`: build `blueprint/reviewer_packet.json` and `blueprint/reviewer_prompt.md`.
7. `run-blueprint`: invoke reviewer in read-only mode; validate and store `blueprint/reviewer_blueprint.json`.
...
15. Invoke reviewer in read-only mode; validate and store `review_decision.json`.
...
- Externalize these keys: `ORCH_WORKSPACE`, `ORCH_STATE_ROOT`, `ORCH_REPORTS_DIR`, `ORCH_MODE`, `ORCH_MAX_CYCLES`, `ORCH_ALLOW_DIRTY`, `ORCH_STAGE_RETRIES`, `ORCH_REQUIRE_HUMAN_ON_FIRST_WRITE`, `ORCH_STOP_ON_VALIDATION_FAIL`, `ORCH_REVIEWER_PROVIDER`, `ORCH_REVIEWER_BIN`, `ORCH_REVIEWER_MODEL`, `ORCH_REVIEWER_PROFILE`, `ORCH_REVIEWER_REASONING`, `ORCH_REVIEWER_SANDBOX`, `ORCH_REVIEWER_APPROVAL`, `ORCH_REVIEWER_TIMEOUT_SEC`, and the matching `ORCH_DRAFTER_*` set.
- `codex` reviewer pattern: `/usr/bin/timeout "$ORCH_REVIEWER_TIMEOUT_SEC" "$ORCH_REVIEWER_BIN" exec --cd "$ORCH_WORKSPACE" --model "$ORCH_REVIEWER_MODEL" --profile "$ORCH_REVIEWER_PROFILE" --sandbox read-only --ephemeral --json --output-schema "$SCHEMA_REVIEW" -o "$LAST_JSON" < "$PROMPT_MD" > "$EVENTS_JSONL" 2> "$STDERR_LOG"`.
- `codex` drafter pattern: `/usr/bin/timeout "$ORCH_DRAFTER_TIMEOUT_SEC" "$ORCH_DRAFTER_BIN" exec --cd "$ORCH_WORKSPACE" --model "$ORCH_DRAFTER_MODEL" --profile "$ORCH_DRAFTER_PROFILE" --sandbox "$ORCH_DRAFTER_SANDBOX" --ephemeral $CODEX_APPROVAL_FLAG --json --output-schema "$SCHEMA_DRAFT" -o "$LAST_JSON" < "$PROMPT_MD" > "$EVENTS_JSONL" 2> "$STDERR_LOG"`.
- `codex` approval mapping should be explicit: `full_auto -> --full-auto`; `dangerous -> --dangerously-bypass-approvals-and-sandbox`; reviewer read-only mode uses no approval flag; unsupported headless approval values should be rejected, not guessed.
- `codex` reasoning should be driven by `--profile` plus optional adapter-specific config overrides; `codex exec` does not expose a dedicated `--effort` flag in this install.
- `claude` fallback pattern: `PROMPT="$(cat "$PROMPT_MD")"; /usr/bin/timeout "$TIMEOUT" "$BIN" --bare --no-session-persistence --add-dir "$ORCH_WORKSPACE" -p "$PROMPT" --output-format json --model "$MODEL" --permission-mode "$PERMISSION_MODE" --effort "$EFFORT" > "$FINAL_JSON" 2> "$STDERR_LOG"`.
- `gemini` fallback pattern: `PROMPT="$(cat "$PROMPT_MD")"; /usr/bin/timeout "$TIMEOUT" "$BIN" --prompt "$PROMPT" --output-format json --model "$MODEL" --approval-mode "$APPROVAL_MODE" $GEMINI_SANDBOX_FLAG > "$FINAL_JSON" 2> "$STDERR_LOG"`.
- `gemini` reasoning should be treated as `provider_default`; no direct effort flag is available in this install.
...
- First bootstrap mode should be `plan-only` with `codex` reviewer only; no write-capable drafter invocation, no commit behavior, no repo mutation beyond future script creation itself.
...
3. Implement the `codex` adapter first, including read-only reviewer invocation, write-capable drafter invocation, output-schema handling, and raw log capture.
4. Implement prompt rendering and JSON contract validation with `jq -e`; reject malformed agent outputs immediately.
5. Implement baseline-vs-current repo delta capture so the controller can operate safely even when the repo starts dirty.
6. Implement reviewer blueprint mode and `plan-only` end-to-end before adding any mutating draft path.
7. Implement the draft path, validation runner, and review cycle loop with `ORCH_REQUIRE_HUMAN_ON_FIRST_WRITE=1` hard-enabled.
8. Add final markdown report generation under `docs/archive/reports/`.
9. After the `codex` path is proven, add `.orchestrator/` ignore rules and switch canonical runtime root from `/tmp/...` to repo-local.
10. Then add `claude` adapter, then `gemini` adapter, without changing the controller state model or file contracts.
